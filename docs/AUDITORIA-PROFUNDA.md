...
> **Verificación:** 37 hallazgos resueltos (+1 parcial) + 1 refutado, con revisión adversarial. `tsc --noEmit` limpio · eslint sin warnings nuevos (errores React Compiler + warnings `<img>`/`outcome` pre-existentes) · suite global **542 tests** verde · `next build` OK.
