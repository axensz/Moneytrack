    ...
    <div className="card">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        <div className="bg-purple-50 dark:bg-purple-900/20 rounded-xl p-3 text-center">
          <Target className="mx-auto text-purple-600 dark:text-purple-400 mb-1" size={20} />
          <p className="text-xs text-gray-600 dark:text-gray-400">Metas activas</p>
          <p className="text-lg font-bold text-purple-700 dark:text-purple-300">{goalStats.activeCount}</p>
        </div>
        <div className="bg-blue-50 dark:bg-blue-900/20 rounded-xl p-3 text-center">
          <p className="text-xs text-gray-600 dark:text-gray-400">Objetivo total</p>
          <p className="text-sm font-bold text-blue-700 dark:text-blue-300">{formatCurrency(goalStats.totalTarget)}</p>
        </div>
        <div className="bg-green-50 dark:bg-green-900/20 rounded-xl p-3 text-center">
          <p className="text-xs text-gray-600 dark:text-gray-400">Ahorrado</p>
          <p className="text-sm font-bold text-green-700 dark:text-green-300">{formatCurrency(goalStats.totalSaved)}</p>
        </div>
        <div className="bg-amber-50 dark:bg-amber-900/20 rounded-xl p-3 text-center">
          <Trophy className="mx-auto text-amber-600 dark:text-amber-400 mb-1" size={20} />
          <p className="text-xs text-gray-600 dark:text-gray-400">Completadas</p>
          <p className="text-lg font-bold text-amber-700 dark:text-amber-300">{goalStats.completedCount}</p>
        </div>
      </div>
            ...
            >
              ...
              <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-3 mb-2">
                <div
                  className={`h-3 rounded-full transition-all duration-500 ${
                    percentage >= 100 ? 'bg-green-500' : percentage >= 75 ? 'bg-blue-500' : 'bg-purple-500'
                  }`}
              ...
              {suggestedMonthly && (
                <p className="text-xs text-blue-600 dark:text-blue-400 mt-1">
                  💡 Ahorra {formatCurrency(suggestedMonthly)}/mes para alcanzar tu meta
                </p>
              )}
