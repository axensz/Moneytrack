...
> **Verificación:** 13 hallazgos resueltos con revisión adversarial. `tsc --noEmit` limpio + suite global **517 tests** verde.
> **Organización:** todo consolidado en la rama **`auditoria-profunda`** → PR único contra `main` (las ramas temáticas originales se eliminaron tras combinar).
> **Pendiente:** atomicidad estricta de C1 (hay harness de Firestore que la apoya), correctness de dinero menor (F-debt-cascade atómico, F-edit-saldo, F-cuotas-redondeo), y la deuda estructural (`FinanceContext` monolítico, dos sistemas de botones, god-files `HelpModal`/`ImportTransactionsModal`).
