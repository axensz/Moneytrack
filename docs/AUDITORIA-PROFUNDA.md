...
> **Verificación:** 8 hallazgos resueltos con revisión adversarial. Cada rama: `tsc --noEmit` limpio + tests verdes.
> **Organización (una rama por TIPO de mejora):** `fix/finanzas-stats-deudas` (#2,#3,#4,#10), `fix/seguridad-backup` (C1), `test/a2-firestore-write-path` (A2), `test/a1-deudas-codigo-real` (A1), `docs/auditoria-profunda` (este informe). Aún sin push.
> **Pendiente:** A3 (tests de los 6 servicios de umbral — `PaymentMonitor === 0/1/3`, etc.), A5 (a11y de modales destructivos: focus-trap + Escape), y la atomicidad estricta de C1 (ya hay harness de Firestore mockeado que la apoya).
