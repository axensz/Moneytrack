...
- [ ] 12.2 Add failing JVM/resource-contract tests for the canonical launcher/splash asset and `MoneyTrack` label; unprefixed AppCompat `colorAccent`; neutral appearance icon and dialog actions; sentence-case primary/secondary/tertiary treatments; compact success state; standalone MoneyTrack utility row; `SwitchCompat`; production filtering of `com.android.shell`; and fallback `Aplicación detectada`.
- [ ] 12.3 Replace the independent Android wallet mark with launcher, adaptive icon and SplashScreen resources derived from the canonical PWA asset; keep startup tied only to real session/preference resolution and add no artificial delay.
- [ ] 12.4 Map AppCompat and platform accent attributes to the existing violet tokens in light/dark, replace the permanent sun with a neutral appearance/contrast icon, and keep `Apariencia` as a native `Sistema / Claro / Oscuro` dialog with explicit `Cancelar` and `Guardar`.
- [ ] 12.5 Apply the semantic action hierarchy across `SESSION`, `NOTIFICATION_ACCESS`, `CAPTURE` and `READY`: one primary violet action, outlined/tonal secondary actions, tertiary sign-out, sentence case, visible enabled states, compact success treatment, success capture panel and a bordered MoneyTrack utility row.
- [ ] 12.6 Use `SwitchCompat` only for capture on/off and checkboxes for source multi-selection; preserve Google Wallet as recommended but unchecked, exclude `com.android.shell` from the production selector/summary, retain an optional diagnostic label and use `Aplicación detectada` for unsafe labels.
- [ ] 12.7 Make Google sign-in single-flight with visible in-progress state and a nearby actionable failure; keep internal configuration details, credential values and exception text out of production UI/logs.
- [ ] 12.8 Add adaptive dimension resources for compact, landscape and expanded widths while retaining the edge-to-edge `ScrollView`; verify 48 dp targets, long labels, bounded source dialog, logical TalkBack order/names, visible focus, AA contrast and no custom motion that ignores reduced motion.
- [ ] 12.9 Run focused RED/GREEN JVM/resource tests, then `testDebugUnitTest lintDebug assembleDebug` with the configured JDK; accept no introduced accessibility, security or layout error and inspect every remaining warning.
...
- [ ] 12.11 Run code-review-graph change/flow/test coverage, strict OpenSpec validation and `git diff --check`; commit only the reviewed Android UX slice and leave the OpenSpec change unarchived until the financial canary gates also pass.
