...
Android calculará `candidateId = SHA-256(deviceInstallId | packageName | statusBarNotification.key | deliveryStartedAt)` como 64 caracteres hexadecimales. `deliveryStartedAt` se ancla al primer `postTime` de esa entrega activa y no cambia con sus actualizaciones. La instalación conserva en preferencias privadas el UUID aleatorio, los hashes de entregas activas y el primer candidato normalizado de cada entrega; nunca guarda la clave cruda, el UID crudo ni el texto original. Cada snapshot y estado de sincronización se asocia al hash del UID autenticado que observó la compra: solo esa sesión puede verlo o reintentarlo, y una entrega todavía activa no se reasigna si cambia la cuenta. Ese primer payload completo se reutiliza en todos los reintentos para que una actualización de Android no intente modificar el documento inmutable de Firestore. La generación se elimina al recibir la remoción o al reconciliarla como ausente de `activeNotifications`; no se divide por antigüedad mientras Android aún la reporte activa. `deviceInstallId` no se sube como campo, el contenido financiero no forma parte del path y nada sensible se imprime en logs.
...
Cada `AccountCard` de un usuario autenticado ofrecerá un botón de icono `Smartphone`, sin rótulo visible y con nombre accesible específico para la cuenta. La acción abrirá un único diálogo `Medios de pago · <cuenta>`, filtrado por su `accountId`; no existirá una sección global ni un panel desplegado debajo de la tarjeta. La lista y los formularios de creación/edición serán vistas del mismo diálogo, sin apilar un segundo modal, y permitirán crear, editar, activar/desactivar y eliminar asociaciones mediante alias, tipo, red y últimos cuatro. `TransactionsView` incorporará antes de los filtros una bandeja colapsable con contador y diálogo de revisión únicamente cuando existan candidatos pendientes; con cero pendientes no ocupará espacio y cualquier error de carga seguirá visible como alerta independiente.
...
El sistema conservará una sola Activity y resolverá una etapa visible a partir del estado real: `SESSION`, `NOTIFICATION_ACCESS`, `CAPTURE` o `READY`. El splash oficial comparte la marca canónica, el violeta y las superficies de la PWA mientras se consulta sesión y preferencias; no introduce una espera artificial. Si todos los requisitos siguen satisfechos, una apertura posterior entra directamente a `READY`. Si una sesión o permiso se pierde, reaparece únicamente la primera etapa incompleta.

Cada etapa explica en pocas líneas para qué sirve su acción. La etapa de sesión muestra solo inicio de sesión; con sesión activa, el inicio desaparece y queda disponible cerrar sesión. La etapa de acceso abre los ajustes oficiales de Android. La etapa de captura usa un `SwitchCompat` para activar o desactivar la captura y casillas únicamente para la selección múltiple de fuentes. Permite elegir Google Wallet como fuente conocida recomendada y etiquetas sanitizadas de aplicaciones observadas, siempre marcadas `No verificada` y sin exponer paquetes técnicos; una colisión normalizada con una fuente conocida cae a `Aplicación detectada`. `READY` cambia el progreso por “Configuración completa” y reduce el bloque operativo a `Captura activa`, las etiquetas elegidas y una única acción `Administrar aplicaciones`, sin repetir una explicación ni el rótulo `Aplicaciones elegidas`. Esta abre un diálogo sobre la misma Activity con explicación y casillas para la fuente conocida y todas las observadas; cancelar no persiste, guardar aplica exactamente la selección y un allowlist vacío devuelve a `CAPTURE`. La lista ajusta su altura con hasta dos fuentes y se vuelve desplazable desde tres. El diálogo aclara que una fuente adicional aparece después de emitir una notificación futura, sin inventariar todas las aplicaciones instaladas. La PWA canónica `https://axensz.github.io/Moneytrack/` se abre desde una fila de utilidad compacta, con padding, borde, etiqueta visible e icono vectorial externo; queda fuera del bloque de captura, no incorpora una explicación redundante y no compite como acción principal. La apariencia `Sistema`, `Claro` u `Oscuro`, con `Sistema` como valor inicial, se elige desde un botón accesible de 48 dp con un icono neutro de apariencia/contraste en la cabecera y no ocupa un bloque permanente del contenido.

La ventana usa edge-to-edge con insets de barras, recortes y navegación gestual aplicados al contenedor desplazable. Ningún título, acción o contenido puede quedar debajo de la barra de estado o de navegación. El contenido permanece vertical y desplazable; títulos flexibles, etiquetas largas, botones de ancho completo y bloques operativos deben adaptarse a pantallas compactas, orientación horizontal y escalado de fuente sin desbordamiento. Colores claros/oscuros replican los roles semánticos existentes y mantienen objetivos de 48 dp, foco y contraste AA.

### 13. Refinar la experiencia Android sin reemplazar su arquitectura

La dirección aprobada es mejorar la Activity AppCompat existente. Una migración a Compose o Material 3, una shell tipo dashboard y una dependencia visual nueva se descartan porque amplían el cambio sin mejorar la captura. La implementación reutiliza vistas XML, recursos DayNight, drawables vectoriales, `SwitchCompat`, `AlertDialog` y los tokens existentes; solo se extraerá lógica pura cuando permita una prueba enfocada.

La identidad Android se deriva del activo canónico de la PWA (`public/moneytrack-icon.svg` y su variante maskable), no de un símbolo de wallet independiente. Launcher, icono adaptativo y SplashScreen deben reconocer la misma marca; el splash espera únicamente la resolución real de sesión y preferencias. El nombre visible usa de forma consistente `MoneyTrack` y el contenido permanece en español.

La jerarquía de interacción aplica estas reglas en todos los estados:

- existe como máximo una acción primaria violeta por estado; las secundarias son delineadas o tonales y las terciarias son texto;
- las etiquetas usan estilo oración, no mayúsculas sostenidas, y un control habilitado nunca se presenta con el gris reservado a estados deshabilitados;
- `Cerrar sesión` aparece únicamente con sesión activa, como acción terciaria separada del recorrido principal;
- `Abrir MoneyTrack` es navegación externa secundaria en su propia fila de utilidad y `Administrar aplicaciones` es la acción principal del estado listo;
- todo resultado activo, completado, pendiente o fallido combina icono y texto; el estado activo conserva una superficie neutral y los estados completado o fallido usan su par semántico, sin depender únicamente del color.

En `READY`, el progreso completado se contrae a un tratamiento de éxito pequeño con `Configuración completa`. El panel principal usa la misma superficie neutral del resto de la aplicación y muestra `Captura activa` mediante icono y texto de éxito, sus fuentes elegidas y una sola acción para administrarlas. Un fallo de escritura persistente oculta ese encabezado y presenta un aviso genérico sólido-sobre-muted hasta que otra escritura se almacene; notificaciones irrelevantes no lo borran. El texto de privacidad es corto y neutral; no parece un enlace violeta ni repite sinónimos como “guardar” y “registrar”. No se presentan `confidence`, `sourcePackage`, `com.android.shell` ni otra metadata de diagnóstico como lenguaje de producto.

Google Wallet permanece siempre disponible como `Recomendada`, pero nunca se activa sin consentimiento. `com.android.shell` puede conservarse en una variante de diagnóstico, etiquetada `Fuente de prueba`, pero se excluye del selector y del resumen de producción. Las etiquetas observadas eliminan controles y marcas bidireccionales, se presentan como `No verificada` y, si están ausentes, equivalen al paquete o colisionan con una fuente conocida, usan `Aplicación detectada`; ningún fallback afirma que la fuente sea financiera.

El control de cabecera abre un diálogo titulado `Apariencia` con `Sistema`, `Claro` y `Oscuro`, además de `Cancelar` y `Guardar`. El icono no representa falsamente el modo actual mediante un sol permanente. Los temas AppCompat declaran tanto `colorAccent` como los atributos de plataforma requeridos y los enlazan al violeta semántico en claro y oscuro, para que radios, foco y acciones no hereden el cian del dispositivo.

El inicio de sesión bloquea gestos repetidos mientras Credential Manager está abierto, comunica `Iniciando sesión…` y presenta el fallo junto a la acción con una salida reparable. Detalles de configuración, tokens y excepciones internas continúan fuera de la UI y de los logs. Los Toast pueden complementar, pero no ser el único canal para un error accionable.
