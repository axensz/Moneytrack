    ...
    return creditCards
      .map(card => {
        ...
        cardTransactions.forEach(t => {
          ...
          // Primera cuota se cobra el mes SIGUIENTE a la compra (ciclo de facturación)
          const monthsSinceFirstInstallment = (currentYear - txDate.getFullYear()) * 12 + (currentMonth - txDate.getMonth()) - 1;

          // 1. Cálculo Mensual: ¿Está activa la deuda este mes?
          if (monthsSinceFirstInstallment >= 0 && monthsSinceFirstInstallment < t.installments) {
          ...
          for (let month = 0; month < 12; month++) {
            const monthsDiff = (currentYear - txDate.getFullYear()) * 12 + (month - txDate.getMonth()) - 1;
        ...
        const pendingInterest = cardTransactions.reduce((sum, t) => {
          if (t.installments && t.totalInterestAmount) {
            const txDate = new Date(t.date);
            // Primera cuota es el mes siguiente a la compra
            const monthsPassed =
