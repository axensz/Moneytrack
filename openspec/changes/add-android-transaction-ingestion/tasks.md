...
- [x] 11.7 Run focused RED/GREEN web and Android tests, then web typecheck/lint and Android `testDebugUnitTest lintDebug assembleDebug`; resolve every introduced warning or failure before device work.
...
- [ ] 11.10 Refine `READY` with safe display labels, one responsive modal source manager, `Configuración completa`, a compact appearance dialog, no repeated source copy and a standalone secondary MoneyTrack web action; verify responsive behavior, source toggles and both dialogs on the authorized canary device.
- [x] 11.11 Add a pure known-source catalog that always offers Google Wallet as recommended but unchecked until explicit consent, merges it once with observed sources, persists the user's choice and verifies future package filtering without `QUERY_ALL_PACKAGES`.
- [x] 11.12 Add RED resource-layout tests for the concise ready block and standalone web action; remove the repeated explanation, `Aplicaciones elegidas` heading and explanatory web card, add a visible vector external-link affordance, then rerun the focused tests GREEN.

## 12. Approved whole-app Android UI/UX refinement

Section 12 refines the existing single-Activity companion; it does not reopen the PWA architecture, ledger boundary or notification contracts. Tasks 11.8 and 11.10 remain incomplete until the combined device matrix below supplies their missing evidence.

- [x] 12.1 Amend `proposal.md`, `design.md`, this Android delta and the task list with the approved UI UX Pro Max audit: canonical brand, native semantic controls, one-primary-action hierarchy, source privacy, auth feedback, adaptive gutters and accessibility matrix; run strict OpenSpec validation before production edits.
- [x] 12.2 Add failing JVM/resource-contract tests for the canonical launcher/splash asset and `MoneyTrack` label; unprefixed AppCompat `colorAccent`; neutral appearance icon and dialog actions; sentence-case primary/secondary/tertiary treatments; compact success state; standalone MoneyTrack utility row; `SwitchCompat`; production filtering of `com.android.shell`; and fallback `Aplicación detectada`.
- [x] 12.3 Replace the independent Android wallet mark with launcher, adaptive icon and SplashScreen resources derived from the canonical PWA asset; keep startup tied only to real session/preference resolution and add no artificial delay.
- [x] 12.4 Map AppCompat and platform accent attributes to the existing violet tokens in light/dark, replace the permanent sun with a neutral appearance/contrast icon, and keep `Apariencia` as a native `Sistema / Claro / Oscuro` dialog with explicit `Cancelar` and `Guardar`.
- [x] 12.5 Apply the semantic action hierarchy across `SESSION`, `NOTIFICATION_ACCESS`, `CAPTURE` and `READY`: one primary violet action, outlined/tonal secondary actions, tertiary sign-out, sentence case, visible enabled states, compact success treatment, neutral active-capture panel with semantic icon/text and a bordered MoneyTrack utility row.
- [x] 12.6 Use `SwitchCompat` only for capture on/off and checkboxes for source multi-selection; preserve Google Wallet as recommended but unchecked, exclude `com.android.shell` from the production selector/summary, retain an optional diagnostic label, mark observed labels `No verificada` and use `Aplicación detectada` for unsafe or colliding labels.
- [x] 12.7 Make Google sign-in single-flight with visible in-progress state and a nearby actionable failure; keep internal configuration details, credential values and exception text out of production UI/logs.
- [x] 12.8 Add adaptive dimension resources for compact, landscape and expanded widths while retaining the edge-to-edge `ScrollView`; verify 48 dp targets, long labels, bounded source dialog, logical TalkBack order/names, visible focus, AA contrast and no custom motion that ignores reduced motion.
...
- [x] 13.2 Add failing Android tests for both captured Wallet fixtures, localized grouping, invalid/ambiguous bodies, parser routing, Unicode nickname bounds and v2 candidate metadata; implement `GoogleWalletPurchaseParser` and the smallest package router; rerun green.
- [x] 13.3 Add failing coordinator tests proving `FLAG_GROUP_SUMMARY` never reads raw extras or writes a candidate; implement the summary metadata gate and result code; rerun green.
- [x] 13.4 Add failing Android persistence/repository tests for v2 schema, optional `observedInstrumentLabel`, parser identity and backward-compatible retry decoding; implement local sync-record v3 with v2 read compatibility and exact Firestore fields; rerun green.
...
- [x] 14.3 Add failing Firestore-emulator tests for owner-only v3 creation, exact keys, an existing account, bounded category, server timestamp, immutable payload and terminal transitions; implement fail-closed rules without weakening v1/v2.
- [x] 14.4 Add failing hook/view tests for opaque `reviewAndroid`, exact document loading beyond the 100-item inbox, invalid/missing/terminal handling and URL cleanup; implement the smallest exact-candidate handoff.
- [x] 14.5 Add failing inbox/modal tests for `Revisar gasto rápido`, valid account/category prefill, stale-option fallback and absence of payment-instrument controls; implement the additive responsive review UI.
...
- [x] 15.3 Add failing checker, transport and throttle tests for fixed HTTPS origin, no redirects, timeouts, 32 KiB limit, 24-hour automatic cadence and manual bypass; implement the background query without blocking either Activity.
- [x] 15.4 Add failing integrity, signer and installation-contract tests; implement scoped download, exact size/SHA-256 verification, package/signer continuity, `FileProvider` and official Package Installer handoff.
- [x] 15.5 Add failing shared-state and resource tests for hidden, available, downloading, ready, permission and error states; integrate a secondary accessible updater in `MainActivity` and `QuickExpenseActivity` without disabling their primary actions.
- [x] 15.6 Add a failing deterministic manifest-generator test; configure bootstrap `0.2.0` code 2 and external-only release signing; document clean build, `apksigner`, hash, bootstrap and first code-3 OTA procedures.
- [x] 15.7 Run focused and full web/Android/rules verification, strict OpenSpec, graph impact/tests and `git diff --check`; keep the manifest on code 1 and stop before GitHub Releases or Pages unless compatible web/rules and external publication are explicitly cleared.
