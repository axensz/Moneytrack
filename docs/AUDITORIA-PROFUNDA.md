...
| **Q-godfiles** (ImportTransactionsModal) | ✅ Resuelto (fase A) | 2026-06-09 | **ImportTransactionsModal descompuesto** (planeado /plan + investigado /workflows). Fase A: toda la lógica/estado (12 vars + ~15 handlers + computed) extraída a `src/hooks/useImportWizard.ts` (testeable aislado); tipos a `src/types/import.ts` (`WizardStep`/`ImportParseStats`/`AISuggestion`); `AIDateAdjuster` extraído a `components/modals/import/AIDateAdjuster.tsx`; el modal pasó de **1230 LOC (lógica+UI mezcladas)** a **~560 LOC de presentación pura** que consume el hook. El commit de dinero (`useImportTransactions`: writeBatch + `increment(usedCredit)`) NO se tocó; parsers/dedup/aprendizaje/aiCategorizer reutilizados. Test `useImportWizard.test.ts` (7: toggle, categoría+regla, import→done, reset). **Fase B opcional (diferida):** partir el JSX de presentación en `ImportUploadStep`/`ImportReviewStep`/`ImportDoneStep` + `AISuggestionsPanel` (puro split de UI, sin lógica). |

> **Verificación:** 42 hallazgos resueltos (+1 parcial) + 1 refutado, con revisión adversarial. `tsc --noEmit` limpio · eslint sin warnings nuevos (React Compiler + unused-imports pre-existentes) · suite global **565 tests** verde · `next build` OK.
