...
| **C2-saldo-paginación** | ✅ Resuelto | 2026-06-09 | **CRÍTICO descubierto post-audit (reporte de corrupción de saldos al ajustar).** Causa raíz: los saldos de ahorro/efectivo se derivan de `initialBalance + Σ(txs en memoria)`, pero el listener principal pagina a las **500 más recientes sobre TODAS las cuentas** → (1) con ≥500 txs el saldo mostrado omite el histórico antiguo; (2) cada tx NUEVA (incluida la de "Ajuste de saldo") **expulsa a la más antigua de la ventana** y el saldo salta por el monto expulsado. Repro exacta de los dos casos reportados al centavo (603.088,11 → fija 563.088,89 → quedaba 225.568,89 por expulsión de un ingreso de 337.520; 2.822.798,94 → fija 563.088,89 → quedaba 603.088,89 por expulsión de un gasto de 40.000). Es el hermano de `F-tc-cupo` para cuentas no-TC (las TC estaban protegidas por `usedCredit` persistido). Fix: `useBalanceTransactions` (historial completo vía `useAllTransactions`, **gated por `hasMoreTransactions`** — 0 lecturas extra con <500 txs ni en modo invitado) alimenta `useAccounts` (display + delta del ajuste + totalBalance) y `BalanceMonitor`. Tests: `balancePaginationCorruption.test.ts` (repro exacta + invariante), `balanceAdjustInvariant.test.ts` (matriz 14 escenarios: sin txs / ingresos / gastos / transferencias / pendientes / ajuste previo / doble edición / sin cambio / centavos / formato COP / delta ±/0), `useBalanceTransactions.test.ts` (gate). ⚠️ Trade-off: con ≥500 txs cada alta/baja refetchea el historial completo (mismo costo ya aceptado en Stats); siguiente paso a escala: saldo persistido por cuenta (patrón `usedCredit`) o rollups. |
...
| `bundle_quality.v1` · **sin minificar / comentarios / sin compresión** | high | **Falso positivo (verificado en vivo)** | Bundle de alta calidad, medido con `curl`: chunks **minificados** (Turbopack, 1 línea, nombres manglados), **gzip** activo en HTML y JS (`Content-Encoding: gzip`), **0** comentarios TODO/FIXME/sourcemaps, **code-split** agresivo (los muchos chunks hash = splitting funcionando; dynamic imports para analytics/xlsx/AIChatBot/help). `package.json` `build` = `next build` (prod, no `build:dev`). La "evidencia" es el **mismo excerpt `AIza...RlGU`** reciclado del finding de key — no mide minificación. | Waivear. brotli no es configurable en Pages (CDN de GitHub); gzip ya presente y suficiente. |
| `bundle_quality.v1` · **503 KB / `content_encoding:null`** | medium | **Falso positivo** | `content_encoding:null` = VibeAuditt **no negoció `Accept-Encoding`**; con `gzip` se sirve comprimido (verificado en 2 chunks). El chunk ~503 KB **crudo** (xlsx/recharts) es afinado de rendimiento, ya registrado como `R-xlsx-full`, no seguridad. | Opcional (perf): lazy/trim de xlsx. No bloqueante. |
| `missing_security_headers.v1` · **x-content-type-options** | medium | **No accionable** | GitHub Pages NO envía el header (verificado) y Pages no deja añadirlo. Riesgo bajo aquí: sitio estático que sirve sus propios assets con Content-Type correcto; no hay subida de archivos servidos desde el mismo origen (los XLSX se parsean client-side, nunca se sirven). | Solo cierra con host de headers (`X-Content-Type-Options: nosniff`). |
