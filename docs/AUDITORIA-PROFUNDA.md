...
> **Verificación:** 40 hallazgos resueltos (+1 parcial) + 1 refutado, con revisión adversarial. `tsc --noEmit` limpio · eslint sin warnings nuevos (React Compiler + unused-imports `<img>`/`outcome`/`Transaction`/`transactionAccountIs` pre-existentes) · suite global **550 tests** verde · `next build` OK.
