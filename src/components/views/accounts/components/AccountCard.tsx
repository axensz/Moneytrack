...
export const AccountCard: React.FC<AccountCardProps> = memo(({
  ...
  const getCardClasses = () => {
    const base = 'rounded-xl p-5 transition-all touch-none select-none';

    if (isDragging) {
      return `${base} opacity-50 scale-[0.98] shadow-lg`;
    }

    if (isDragOver) {
      return `${base} border-2 border-primary shadow-md bg-primary/5`;
    }
