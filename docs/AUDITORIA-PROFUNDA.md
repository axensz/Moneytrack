...
|----------|--------|-------|---------------|
| **C1** backup replace | ✅ Mitigado | 2026-06-08 | `useBackup.ts`: write-then-delete (snapshot → escribe nuevo → borra viejo al final). Ya no hay pérdida total; residual: si falla a mitad quedan duplicados recuperables (no atómico estricto — pendiente harness Firestore). Test: `backupReplaceAtomic.test.ts` (3). |
...
| **A5** a11y modales | ✅ Resuelto (parcial) | 2026-06-08 | Nuevo hook `useModalA11y` (scroll-lock, Escape, focus-trap, restauración de foco); `BaseModal` refactorizado a usarlo; aplicado a `DeleteConfirmModal`/`AccountFormModal`/`NotificationPreferencesModal` + `role`/`aria-modal`/`aria-label`. Test `useModalA11y.test.tsx`. **Falta:** modales INLINE en `DebtsView`/`BudgetsView` y los `confirm()` nativos de budgets/goals (pueden adoptar el mismo hook). |

> **Verificación:** 10 hallazgos resueltos con revisión adversarial. Cada rama: `tsc --noEmit` limpio + tests verdes.
> **Organización (una rama por TIPO de mejora):** `fix/finanzas-stats-deudas` (#2,#3,#4,#10), `fix/seguridad-backup` (C1), `test/a2-firestore-write-path` (A2), `test/a1-deudas-codigo-real` (A1), `test/a3-servicios-notificacion` (A3), `fix/a11y-modales-destructivos` (A5), `docs/auditoria-profunda` (este informe). Aún sin push.
> **Pendiente:** completar A5 inline (`DebtsView`/`BudgetsView`/`confirm()` nativos), atomicidad estricta de C1 (ya hay harness de Firestore mockeado), y la deuda estructural (`FinanceContext` monolítico, dos sistemas de botones, god-files).
