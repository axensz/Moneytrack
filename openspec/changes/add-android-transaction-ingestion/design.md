...
- todo resultado activo, completado, pendiente o fallido combina icono y texto con su par semántico; el color nunca comunica el estado por sí solo.

En `READY`, el progreso completado se contrae a un tratamiento de éxito pequeño con `Configuración completa`. El panel principal muestra `Captura activa` con el par success sólido-sobre-muted, sus fuentes elegidas y una sola acción para administrarlas. El texto de privacidad es corto y neutral; no parece un enlace violeta ni repite sinónimos como “guardar” y “registrar”. No se presentan `confidence`, `sourcePackage`, `com.android.shell` ni otra metadata de diagnóstico como lenguaje de producto.

Google Wallet permanece siempre disponible como `Recomendada`, pero nunca se activa sin consentimiento. `com.android.shell` puede conservarse en una variante de diagnóstico, etiquetada `Fuente de prueba`, pero se excluye del selector y del resumen de producción. Una etiqueta ausente o equivalente a su paquete se presenta como `Aplicación detectada`; ningún fallback afirma que la fuente sea financiera.
