...
> **Verificación:** 13 hallazgos resueltos con revisión adversarial. `tsc --noEmit` limpio + suite global **518 tests** verde.
...
> **Pendiente:** correctness de dinero menor (`deleteDebt` atómico/F-debt-cascade, F-edit-saldo, F-cuotas-redondeo), y la deuda estructural (`FinanceContext` monolítico, dos sistemas de botones, god-files `HelpModal`/`ImportTransactionsModal`).
