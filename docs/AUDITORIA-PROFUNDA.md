...
> **Actualización 2026-07-26:** el importador de extractos CSV/XLSX/PDF fue retirado. Los hallazgos históricos sobre ese subsistema y la dependencia `xlsx` ya no describen funcionalidad activa.
>
...
| Q-godfiles | God-file pendiente: `HelpModal.tsx` (1340 LOC de JSX estático ≈ 3.6% del src). El antiguo wizard de importación fue eliminado. | `wc -l`; `HelpModal.tsx:10` | Media |
...
| UI-touch-44 | El piso de 44px de touch target es solo móvil y excluye `.text-xs` → varios icon/chip buttons quedan bajo 44px; desktop sin piso. WCAG 2.5.5. | `utilities.css:272-278`; `FilterDropdown.tsx:55` | Media |
...
| R-xlsx-full | ✅ Eliminado junto con el importador de extractos y la dependencia `xlsx`. | `package.json`; `package-lock.json` | Resuelto |
...
3. God-files (`HelpModal` 1340; la orquestación Firestore de `useAccounts` ya fue extraída).
4. La orquestación peligrosa de cuentas ya está aislada en `hooks/firestore/accountOrchestration.ts`; aún conviene unificar gradualmente otros hooks de feature que escriben directo.
...
- Descomponer `HelpModal`; mantener cascade/merge fuera del hook de UI.
...
| **F-debt-cascade** | ✅ Resuelto | 2026-07-26 | `useDebts.deleteDebt` (autenticado) adquiere el lease global, relee deuda/transacciones/cuentas desde el servidor y borra las transacciones vinculadas + revierte `usedCredit` + borra la deuda + libera el lease en **UN solo batch**. La liberación usa un tombstone verificable para impedir que un commit retrasado libere un lock nuevo. Se rechaza antes de escribir si el total supera 40 operaciones; ya no existe fallback secuencial parcial. Las reglas también impiden crear o mover una transacción hacia un `debtId` inexistente. Tests: `debtCascadeDelete.test.ts` (4). |
...
| **C2-saldo-paginación** | ✅ Resuelto | 2026-07-26 | **CRÍTICO descubierto post-audit (reporte de corrupción de saldos al ajustar).** Causa raíz: los saldos de ahorro/efectivo se derivan de `initialBalance + Σ(txs en memoria)`, pero el listener principal pagina a las **500 más recientes sobre TODAS las cuentas** → (1) con ≥500 txs el saldo mostrado omite el histórico antiguo; (2) cada tx NUEVA expulsa a la más antigua de la ventana y el saldo salta por el monto expulsado. Fix vigente: `useBalanceTransactions` activa una suscripción perezosa al historial completo cuando la colección llega a 500 filas o conserva más páginas; el primer snapshot confirmado por servidor es la autoridad y los siguientes cambios llegan como deltas. El listener permanece activo aunque `hasMoreTransactions` pase a falso, por lo que ediciones/borrados antiguos de otra pestaña no quedan obsoletos. Tests: `balancePaginationCorruption.test.ts`, `balanceAdjustInvariant.test.ts`, `useAllTransactions.test.ts` y `useBalanceTransactions.test.ts`. ⚠️ Trade-off: el primer acceso con historiales grandes todavía sincroniza la colección completa; la siguiente escala requiere saldos persistidos/rollups. |
...
| **R-allTx-refetch** | ✅ Resuelto | 2026-07-26 | `useAllTransactions` ya no relee la colección completa por cada alta, baja o edición. Usa un `onSnapshot` perezoso: una sincronización inicial completa y luego `docChanges()` incrementales, incluidos cambios remotos antiguos. Tras el primer snapshot de servidor, el historial completo es autoritativo y además corrige la caché paginada; las escrituras locales pendientes no se propagan prematuramente. Estadísticas reutiliza esta misma fuente, evitando una segunda copia/suscripción completa. |
...
| **Q-context** | ✅ Resuelto | 2026-06-09 | **FinanceContext migrado a store con `useSyncExternalStore`** (planeado con /plan + investigado con /workflows). **Fase 1:** nuevo `src/contexts/financeStore.ts` — `createStore` (snapshot mutable + listeners), `shallowEqual`, `useStoreSelector` sobre el `useSyncExternalStore` NATIVO de React 19 (sin shim) con caché de selección + bail-out. **Fase 2:** `FinanceProvider` crea el store una vez (ref estable) y empuja el `value` memoizado en `useLayoutEffect`; el Context lleva el STORE (identidad estable → nunca re-renderiza solo); `useFinance()` = selector identidad (comportamiento idéntico). **Fase 3:** los consumidores directos de vistas y formularios migraron a hooks por dominio + `useFormatCurrency`; el antiguo wizard fue eliminado posteriormente. **Motor de datos/CRUD y acoplamiento (deleteTransactionWithDebtSync, usedCredit, mergeCreditCards, orden de hooks) INTACTOS.** Tests `financeStore.test.ts` (8). ⚠️ Beneficio runtime modesto (el lazy-mount ya aislaba las vistas pesadas); el valor es el seam arquitectónico correcto. R-provider-eager y R-balance-recalc NO se resuelven aquí (follow-ups). |

| **Q-godfiles** (importador de extractos) | ✅ Eliminado | 2026-07-26 | Se retiró por completo la portada/wizard de importación de transacciones, sus parsers CSV/XLSX/PDF, IA asociada, hooks, tipos, pruebas específicas y la dependencia `xlsx`. Permanecen únicamente exportación CSV, respaldo/restauración y migración de invitado, que son flujos distintos. |

| **Q-useAccounts** | ✅ Resuelto | 2026-07-26 | `useAccounts` conserva composición/validación local y delega la rama autenticada en `accountOrchestration`. Eliminar, fusionar y cambiar la cuenta predeterminada adquieren un lock por usuario con lease basado en `serverTimestamp`/`request.time`, releen el estado actual desde el servidor y confirman un único batch atómico. El tombstone de liberación conserva la identidad del lease y bloquea commits retrasados. El cascade descubre transacciones, pagos recurrentes, deudas y tarjetas huérfanas sin confiar en arrays React; la fusión aborta si el destino cambió remotamente para no perder ediciones. Las reglas impiden revivir referencias hacia cuentas borradas y se reconcilia `usedCredit` dentro del mismo commit. La fusión (updates complejos de transacción) se limita a 15 operaciones; delete/default usan 40. Ambas cotas dejan margen frente al máximo de 1.000 expresiones de reglas; volúmenes mayores requieren una migración administrada. |
...
| **S-xlsx-cdn** | ✅ Eliminado | 2026-07-26 | La portada de importación de extractos y su parser XLSX fueron retirados por decisión de producto; también se eliminó la dependencia `xlsx`. El riesgo y el peso asociado ya no existen en el runtime. |

| **F-tc-paid-doble** | ✅ Resuelto (decisión: correcto, aclarar UI) | 2026-06-09 | **Decisión del usuario: el comportamiento es correcto** (una compra TC pagada es a la vez un gasto —ya gastaste el dinero— y deuda pendiente de la tarjeta hasta pagar el extracto: dos lentes legítimas). Sin cambio de lógica. `StatsCards` ahora aclara con tooltip + icono Info que "Pendientes" incluye la deuda de TC, no solo gastos sin marcar pagados. |
| **S-gemini-plaintext** | ✅ Resuelto (decisión: aceptar + endurecer) | 2026-06-09 | **Decisión (con research): aceptar + documentar, NO cifrar.** Cifrar la key client-side sin passphrase es teatro (la clave de descifrado estaría disponible al mismo JS; XSS la lee igual); passphrase = fricción; backend-proxy = no existe (static-export). Riesgo acotado por diseño: BYOK revocable, owner-scoped por reglas Firestore, y caché IndexedDB **se vacía al cerrar sesión** (`clearFirestorePersistence`, ya cableado). Modelo de amenaza documentado en `useGeminiApiKey.ts`; el `GeminiKeyModal` ya divulga el almacenamiento. |
| **UI-text-small** | 🟡 Parcial (contraste) | 2026-06-09 | Pase conservador de contraste (sin tocar tamaños → cero riesgo de layout): cabeceras de fecha de `TransactionsList` y label de cuenta de `AdjustmentGroup` `text-gray-400`→`text-gray-500` en modo claro (dark intacto), sumado al fix previo de `TransactionItem`. El barrido completo de tamaños 10/11px (~14 archivos) sigue requiriendo pase de diseño visual — **pendiente**. |

| **P-cascade-incons** | ✅ Resuelto (divulgación) | 2026-06-09 | **Decisión del usuario: ConfirmDialog unificado con efectos** (divulgar, sin cambiar la lógica de cascada). El borrado de cuenta (`DeleteConfirmModal`) ahora divulga TODO lo que se borra en cascada — transacciones + pagos recurrentes + deudas vinculadas + reversión de cupo de TC — no solo transacciones. `DeletePaymentModal` ya divulgaba que las tx se conservan, y `DebtsView`/`ConfirmDialog` la cascada de deuda (P-debt-disclosure). Semántica consistente de divulgación en todas las acciones destructivas. Test `deleteConfirmDisclosure.test.tsx` (2). |
| **P-onboarding** | ✅ Resuelto | 2026-06-09 | **Decisión del usuario: checklist de primeros pasos** (pasivo, no tour intrusivo). Nuevo `OnboardingChecklist` (tarjeta dismissable en el shell): 3 pasos que se marcan SOLOS según el estado real (hay cuentas / hay movimientos / IA lista), con CTA a la acción de cada uno. Se oculta al completar todo o al cerrarla (persistido en localStorage). Sustituye la dependencia del manual estático como onboarding. Test `onboardingChecklist.test.tsx` (4). |

> **Verificación:** 48 hallazgos resueltos (+2 parciales, +1 no-fix justificado) + 1 refutado, con revisión adversarial. `tsc --noEmit` limpio · eslint sin warnings nuevos · suite global **575 tests** verde · `next build` OK. **Toda la deuda arquitectónica, de dinero y de UX accionable cerrada.**
>
> **Restantes (no accionables hoy):** `S-replace-orphan` (gated: backup no cableado a UI — se activa al cablearlo, junto con C1 ya resuelto), barrido de TAMAÑOS de `UI-text-small` (pase de diseño visual fino). El resto del informe está resuelto, parcial-documentado, refutado o marcado no-fix.
...
> **Deuda estructural pendiente (requiere decisión):** (1) `FinanceContext` — continuar reduciendo consumidores del contexto completo; (2) **A4 tokens/botones** — adoptar el `<Button>` del design-system como único sistema + tokenizar (decisiones de paleta/gradiente); (3) descomponer `HelpModal`. La orquestación cruda de cuentas ya vive fuera del hook de UI y el importador de extractos ya no existe.

---

## Triage VibeAuditt — escáner externo (2026-06-18)

> Escaneo del auditor externo **VibeAuditt** sobre `https://axensz.github.io/Moneytrack/`. Triage adversarial: cada hallazgo verificado contra el código y el hosting real.
>
> **Raíz común:** el sitio es **export estático de Next servido por GitHub Pages**, que **no sirve headers HTTP personalizados** y **no tiene backend/API propio**. VibeAuditt verifica con `curl -I` (solo lee headers) → no ve ni el CSP en `<meta>` ni el modelo de seguridad real. La mayoría son **falsos positivos en contexto** (key pública por diseño; CSP presente vía meta; bundle minificado+gzip+code-split, verificado; CORS y HSTS benignos) o **no accionables en Pages** (headers que solo GitHub controla). VibeAuditt además **recicla el excerpt `AIza...RlGU`** entre checks distintos (`exposed_api_keys` y `bundle_quality`).
>
> **Dos hallazgos SÍ eran reales y se corrigieron en código** (2026-06-18, [app/layout.tsx](../app/layout.tsx), typecheck OK): **`og_meta_missing`** (OpenGraph + Twitter ausentes) y **`referrer-policy`** (vía `<meta name="referrer">`, que sí funciona en estático).
>
> **Headers que GitHub Pages SÍ envía (verificado con `curl`):** `Strict-Transport-Security: max-age=31556952`, `Content-Encoding: gzip`, `Access-Control-Allow-Origin: *`, y **HTTP→HTTPS 301**. **NO envía:** `X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy`, `Permissions-Policy`, `Content-Security-Policy` (esos no se pueden añadir en Pages).

| Check | Sev. reportada | Veredicto | Causa raíz / evidencia | Acción |
|-------|----------------|-----------|------------------------|--------|
| `missing_security_headers.v1` · **content-security-policy** | high | **Falso positivo** | CSP SÍ presente, fuerte y **confirmado en vivo** vía `<meta http-equiv>` ([app/layout.tsx:44-68](../app/layout.tsx)). Supera el mínimo pedido (`default-src 'self'`): incluye `object-src 'none'`, `base-uri 'self'`, `form-action 'self'`, `connect/frame/img/script` acotados. VibeAuditt mira el header, no el meta. | Waivear. Brechas **solo-header** no cubribles en Pages: `frame-ancestors`, `report-uri`/`report-to`, y `script-src 'unsafe-inline'` (forzado por export estático: sin servidor no hay nonces; documentado [app/layout.tsx:35](../app/layout.tsx)). |
| `exposed_api_keys.v1` · **Google API Key** | high | **Falso positivo** | Es la **Firebase Web API key** (`apiKey: process.env.NEXT_PUBLIC_FIREBASE_API_KEY`, [src/lib/firebase.ts:7](../src/lib/firebase.ts)), **pública por diseño** ([firebase.google.com/docs/projects/api-keys](https://firebase.google.com/docs/projects/api-keys)). No es la key de Gemini (BYOK, solo en navegador del usuario). Los datos los protege [firestore.rules](../firestore.rules) (owner-scoped + default-deny), no el secreto de la key. | **NO rotar** (la nueva sería igual de pública y rompe el deploy). **NO mover server-side** (no hay servidor: `output: 'export'`). Acción opcional **en consola GCP** (fuera del repo): restringir la key por **HTTP referrer** (`axensz.github.io/*`) + **API restrictions** → mata el riesgo de abuso de APIs facturables. |
| `missing_security_headers.v1` · **permissions-policy** | low | **No accionable** | Header-only sin equivalente `<meta>`; GitHub Pages no puede servirlo. La amenaza (iframe pidiendo cámara/geo) ya está mitigada por el CSP `frame-src` ([app/layout.tsx:51](../app/layout.tsx)) — un iframe inyectado ni siquiera carga off-origin. | Waivear. Solo se cierra con host de headers. |
| `cors_misconfig.v1` · **Access-Control-Allow-Origin: \*** | high | **Falso positivo** | **No hay API**: export estático (`output: 'export'`), 0 route handlers / serverless / functions (verificado). El `ACAO: *` lo pone **GitHub Pages** sobre assets estáticos públicos (`Server: GitHub.com`, verificado con `curl`), no el código. Variante peligrosa **ausente**: con `Origin: malicious` devuelve `*` (no refleja) y `allow_credentials:false` → sin robo de credenciales. Datos reales = Firestore (endpoint de Google; auth por **bearer token, no cookie** + `firestore.rules` owner-scoped + default-deny). | Waivear. Nada que cambiar: el header es de GitHub y `*` **sin credenciales** sobre archivos ya públicos no expone nada. |
| `bundle_quality.v1` · **sin minificar / comentarios / sin compresión** | high | **Falso positivo (verificado en vivo)** | Bundle de alta calidad, medido con `curl`: chunks **minificados** (Turbopack, 1 línea, nombres manglados), **gzip** activo en HTML y JS (`Content-Encoding: gzip`), **0** comentarios TODO/FIXME/sourcemaps y code-split para analytics/AIChatBot/help. `package.json` `build` = `next build` (prod, no `build:dev`). La "evidencia" es el mismo excerpt de la key pública de Firebase, que no mide minificación. | Waivear. brotli no es configurable en Pages (CDN de GitHub); gzip ya presente y suficiente. |
| `bundle_quality.v1` · **503 KB / `content_encoding:null`** | medium | **Falso positivo histórico** | `content_encoding:null` significaba que VibeAuditt no negoció `Accept-Encoding`; con `gzip` se sirve comprimido. Además, la dependencia XLSX que contribuía al chunk fue eliminada junto con el importador de extractos. | Volver a medir el deploy siguiente; no queda acción XLSX. |
| `missing_security_headers.v1` · **x-content-type-options** | medium | **No accionable** | GitHub Pages NO envía el header (verificado) y Pages no deja añadirlo. Riesgo bajo aquí: sitio estático que sirve sus propios assets con Content-Type correcto y ya no acepta archivos de extractos. | Solo cierra con host de headers (`X-Content-Type-Options: nosniff`). |
| `missing_security_headers.v1` · **x-frame-options** | medium | **No accionable** | Anti-clickjacking. GitHub no lo envía; header-only, igual que `frame-ancestors` (el `<meta>` lo ignora) → no se puede en Pages. Riesgo acotado: las acciones sensibles requieren auth. | Cierra con host de headers (`X-Frame-Options: DENY` + CSP `frame-ancestors 'none'`). |
| `missing_security_headers.v1` · **referrer-policy** | low | **✅ Corregido (meta)** | El header no estaba, pero Referrer-Policy SÍ tiene forma `<meta name="referrer">` que funciona en estático → añadido vía `metadata.referrer` en [app/layout.tsx](../app/layout.tsx). Nota: el default del navegador ya es `strict-origin-when-cross-origin`; el `curl -I` del scanner (header) **no** lo verá (es meta), pero el comportamiento es correcto. | Hecho. |
| `tls_misconfig.v1` · **HSTS / HTTP→HTTPS** | low | **Falso positivo** | HSTS **presente** (`max-age=31556952`, verificado) + HTTP **301→HTTPS** (verificado) + `.github.io` ya en la **preload list** del navegador. `includeSubDomains`/`preload` los controla GitHub, no configurable. La prosa "acepta HTTP sin redirigir" es falsa. | Waivear. |
| `og_meta_missing.v1` · **OpenGraph / Twitter** | medium/low | **✅ Corregido** | Tags genuinamente ausentes (verificado, `present:[]`). Añadidos `openGraph` (title/description/type/url/siteName/locale/image) + `twitter` (`summary`) vía la metadata API de Next en [app/layout.tsx](../app/layout.tsx). `og:image` reusa el icono PWA 512² (cuadrado → card `summary`); upgrade opcional: `public/og-image.png` 1200×630 + `summary_large_image`. | Hecho. typecheck OK. |

**Único trabajo con peso (opcional, decisión de hosting — no bug de código):** migrar el deploy a un **host que sirva headers** — Firebase Hosting (ya nombrado como destino en [app/layout.tsx:32](../app/layout.tsx)), Cloudflare Pages, Netlify o Vercel. Eso permitiría: (1) mover el CSP de `<meta>` a header real + añadir `frame-ancestors 'self'` y `report-uri`; (2) servir `Permissions-Policy`; (3) bajar `script-src 'unsafe-inline'` a nonces. Mientras sigamos en GitHub Pages, los hallazgos de **headers** (CSP, permissions-policy, CORS, x-content-type-options, x-frame-options, HSTS) se **waivean en VibeAuditt** — no servibles/configurables en Pages (el escaneo de permissions-policy apunta además a `github.com/axensz/Moneytrack`, la página del repo, no a la app). **`og_meta_missing` y `referrer-policy` SÍ se corrigieron en código** ([app/layout.tsx](../app/layout.tsx)); `referrer-policy` igual seguirá apareciendo en el `curl -I` del scanner porque es `<meta>`, no header. Un bloque `headers()` seguiría sin hacer nada bajo `output: 'export'`.
